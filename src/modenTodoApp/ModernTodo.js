...
const ModernTodo = () => {
	...
	const handleOnSubmit = (e) => {
		e.preventDefault();
		if (currentTask.trim().length !== 0) settasks(tasks.concat(currentTask.trim()));
		setcurrentTask('');
	};

	const handleStrikeThroughOnClick = (i) => {
		document.getElementById(i).classList.toggle('task-item');
		if (document.getElementById(i).classList.contains('task-item')) setdoneCounter(doneCounter + 1);
